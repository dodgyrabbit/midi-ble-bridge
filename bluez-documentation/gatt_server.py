...
class TestService(Service):
    ...
    """
    TEST_SVC_UUID = '03B80E5A-EDE8-4B33-A751-6CE34EC4C700'

    def __init__(self, bus, index):
        Service.__init__(self, bus, index, self.TEST_SVC_UUID, True)
        self.add_characteristic(TestCharacteristic(bus, 0, self))
        print('Constructing test service')
...
class TestCharacteristic(Characteristic):
    ...
    """
    TEST_CHRC_UUID = '7772E5DB-3868-4112-A1A9-F2669D106BF3'

    def __init__(self, bus, index, service):
        Characteristic.__init__(
                self, bus, index,
                self.TEST_CHRC_UUID,
                ['write-without-response','read','notify'],
                service)
        self.value = []
    ...
    # From what I can tell, this starts once the device connects to us
    def StartNotify(self):
        if self.notifying:
            print('Already notifying, nothing to do')
            return
        ...
        self.notifying = True
        self.time_ms = current_milli_time()
        self._start_midi_feed()
    ...
    def StopNotify(self):
        ...
        print('Notifications stopping')
        self.notifying = False

        # This looks like a bug - why call it again?
        self._start_midi_feed()

    def _start_midi_feed(self):
        ...
        value.append(dbus.Byte(0x80))
        value.append(dbus.Byte(0x80))

        #value.append(dbus.Byte(timestamphigh))

        # Set highest bit to 1
        #value.append(dbus.Byte(timestamplow))
        value.append(dbus.Byte(0xFE))
        ...
        GObject.timeout_add(1000, self.send_midi_message)
    ...
    def send_midi_message(self):
        ...
        #msgBuf0 = ((currentTimeStamp >> 7) & 0x3F) | 0x80; //6 bits plus MSB
        #msgBuf1 = (currentTimeStamp & 0x7F) | 0x80; //7 bits plus MSB



        bottom13bits = current_time & 0x01FFF
        timestamphigh = ((bottom13bits >> 7) & 0x3F) | 0x80
        timestamplow = (bottom13bits & 0x7F)| 0x80

        # only use bottom byte
        #timestamplow = bottom16 & 0xff

        #  set first bit to 1
        #timestamplow = timestamplow | 0x80
       
        #timestamphigh = bottom16 >> 7

        # mask off top two bits
        #timestamphigh = timestamphigh & 0x3F
        # set first bit
        #imestamphigh = timestamphigh | 0x80

        # Calculate bottom bits
        ...
        value.append(dbus.Byte(0x90))
        value.append(dbus.Byte(0x3C))
        value.append(dbus.Byte(0x36))

        value.append(dbus.Byte(0x90))
        value.append(dbus.Byte(0x3C))
        value.append(dbus.Byte(0x2C))
